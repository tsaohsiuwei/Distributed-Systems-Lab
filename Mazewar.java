...
public class Mazewar extends JFrame {
         ...
         */
        public Mazewar(String serverHost, int serverPort, int peerPort) throws IOException,
				...
				myName = name;
                nSocket = new NSocket(serverHost, serverPort);				
                
				
				//Send hello packet to server
                InetAddress IP = InetAddress.getLocalHost();
                MPacket hello = new MPacket(name, MPacket.HELLO, MPacket.HELLO_INIT,IP, peerPort);
				...
				MPacket resp = (MPacket) nSocket.readObject();
                
                for(int g=0;g<4;g++){
                    //if (resp == null)
                       // System.out.println("shit happens");
                	//System.out.println("IP is: "+resp.IPs[g].toString()+" peerport is: "+ resp.ports[g]);
                    info.IPs[g] = resp.IPs[g];
                    info.ports[g] = resp.ports[g];
                    info.acks[g] = false;
                    info.names[g] = resp.players[g].name; 
                }
        ...
        */
        private void startThreads(int peerPort){
                //Start a new sender thread 
                //new Thread(new ClientSenderThread(nSocket, eventQueue)).start();
                //Start a new listener thread 
                //new Thread(new ClientListenerThread(nSocket, clientTable)).start();
               try{
				new Thread(new PeerConnectionThread(peerPort, eventQueue, myName, clientTable, info)).start();
                //MSocket[] mSocketList = null;
               // mSocketList = new MSocket[4];
                Thread.sleep(4000);
				for (int i=0; i<4; i++)
				{
					if (info.names[i].equals(myName))
                        ...
                        String host = info.IPs[i].toString(); 
                        System.out.println("host is: "+host);
                        //String[] realHost= new String[2];
                        //realHost = host.split(".",2);
                        String sub = host.substring(0,5); 
						System.out.println("realhost is: "+ sub);
                     
                        MSocket s = new MSocket(sub, info.ports[i]);
                        int local = s.port;
                        info.locals[i]= local;
                        new Thread(new PeerClientListenerThread(s, clientTable, info, myName)).start();
        ...
        public static void main(String args[]) throws IOException,
             ...
             /* Create the GUI */
             Mazewar mazewar = new Mazewar(host, port, peerPort);
             mazewar.startThreads(peerPort);
        }
}
